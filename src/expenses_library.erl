...
get_account_transactions(AccountId, Start, End) when is_integer(Start), is_integer(End) ->
  try
    ...
    case cqerl:get_client({}) of
      {ok, Client} ->
        case cqerl:run_query(Client, #cql_query{statement = <<"SELECT transaction_id,date,description,amount,category,sub_category,tags FROM transactions WHERE account_id = ? AND date >= :start_date AND date <= :end_date">>, values = [{account_id, AccountIdUUID}, {start_date, Start}, {end_date, End}], page_size = 50000}) of
          {ok, Result} ->
